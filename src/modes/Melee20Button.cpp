...
void Melee20Button::SetConfig(
    GameModeConfig &config,
    const MeleeOptions options,
    const bool cwos,
    const uint32_t modx_x_wd,
    const uint32_t modx_y_wd,
    const uint32_t mody_x_wd,
    const uint32_t mody_y_wd
) {
    InputMode::SetConfig(config);
    _options = options;
    _cwos = cwos;
    _modx_x_wd = modx_x_wd;
    _modx_y_wd = modx_y_wd;
    _mody_x_wd = mody_x_wd;
    _mody_y_wd = mody_y_wd;
}
...
void Melee20Button::UpdateDigitalOutputs(const InputState &inputs, OutputState &outputs) {
    ...
    if (inputs.nunchuk_connected) {
        outputs.start = inputs.nunchuk_start || inputs.mb1;
...
void Melee20Button::UpdateAnalogOutputs(const InputState &inputs, OutputState &outputs) {
    ...
    bool shield_button_pressed =
        inputs.lf4 || inputs.rf5 || inputs.rf7 || inputs.rf8 || inputs.nunchuk_z;
    ...
    if (inputs.lt1) {
        ...
        if (directions.diagonal && shield_button_pressed) {
            ...
            if (_options.has_custom_airdodge) {
                ...
                outputs.leftStickX = 128 + (directions.x * _modx_x_wd);
                outputs.leftStickY = 128 + (directions.y * _modx_y_wd);
            }
        }
    ...
    if (inputs.lt2) {
        ...
        if (directions.diagonal && shield_button_pressed) {
            ...
            if (directions.y == -1) {
                /*if (_options.has_short_custom_airdodge) {
                    // outputs.leftStickX = 128 + (directions.x * _options.short_custom_airdodge.x);
                    // outputs.leftStickY = 128 + (directions.y * _options.short_custom_airdodge.y);
                } else { */
                outputs.leftStickX = 128 + (directions.x * _mody_x_wd);
                outputs.leftStickY = 128 + (directions.y * _mody_y_wd);
                //}
            }
        }
    ...
    if (inputs.rf8 && !inputs.nunchuk_connected) {
        outputs.triggerRAnalog = 94;
    }

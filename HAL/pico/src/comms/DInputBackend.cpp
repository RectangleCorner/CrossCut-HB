...
const uint8_t DInputBackend::ps3_magic_init_bytes[8] = { 0x21, 0x26, 0x01, 0x07,
                                                         0x00, 0x00, 0x00, 0x00 };
...
void DInputBackend::SendReport(bool isMelee, bool travelTime) {
    if (!isMelee) {
    ...
    if (isMelee == true) {
        if (travelTime == true) {
            LimitOutputs(125); // quarterInterval magic number from config.cpp
        } else {
            LimitOutputsHax(125);
        }
    }
    ...
    // Digital outputs
    if (!isMelee) {
        //_gamepad.sendRawData(ps3_magic_init_bytes, sizeof(ps3_magic_init_bytes));
        // Digital outputs
        ...
        // D-pad Hat Switch
        _gamepad
            .hatSwitch(_outputs.dpadLeft, _outputs.dpadRight, _outputs.dpadDown, _outputs.dpadUp);
    } else {
        // Digital outputs
        ...
        // D-pad Hat Switch
        _gamepad.hatSwitch(
            _finalOutputs.dpadLeft,
            _finalOutputs.dpadRight,
            _finalOutputs.dpadDown,
            _finalOutputs.dpadUp
        );
    }
    _gamepad.sendState();
}

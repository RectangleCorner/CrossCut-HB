...
GamecubeBackend::GamecubeBackend(
    ...
    _rumble_pin = rumble_pin;
    _rumble_option = rumble_option;
}
...
void GamecubeBackend::SendReport(bool isMelee, bool travelTime) {
    ...
    if (detect) {
        ...
        loopCount++;
        if (loopCount > 5 &&
            loopTime > 300) { // screen out implausibly fast samples; the limit is 2500 Hz (400 us)
            minLoop = min(minLoop, loopTime);
        }
        if (loopCount >= 100) {
            detect = false;
            sampleCount = 1;
            while (1000 * sampleCount <= minLoop
            ) { // we want [sampleCount] ms-spaced samples within the smallest possible loop
                sampleCount++;
            }
        ...
        // we want the last sample to begin [850 + extra computation time] before the beginning of
        // the last poll to give room for the sample and the travel time+nerf computation
        //
        for (uint i = 0; i < sampleCount; i++) {
            ...
            if (isMelee && travelTime) {
                // APPLY NERFS HERE
                OutputState nerfedOutputs;
                limitOutputs(sampleSpacing / 4, AB_A, _inputs, _outputs, nerfedOutputs);
    ...
    if (rumble_status != PollStatus::ERROR) {
        ...
        // if (rumble_status == PollStatus::RUMBLE_ON && _rumble_option == 1) {
        if (rumble_status == PollStatus::RUMBLE_ON && _rumble_option) {
            gpio_put(_rumble_pin, 1);
            gccRumble = true;
        } else {
            gpio_put(_rumble_pin, 0);
            gccRumble = false;
        }
    }
}
